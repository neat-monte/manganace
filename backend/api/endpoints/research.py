...
@router.get('/{r_session_id}/trials', response_model=List[TrialMeta])
def get_trials_meta_info(r_session_id: int, done: bool = False, db: Session = Depends(get_db)) -> Any:
    ...
    db_session_r = data.session_r.get(db, r_session_id)
    if not db_session_r:
        raise HTTPException(status_code=400, detail="Session not found")
    return services.trial.get_trials_meta(db, db_session_r, done)
