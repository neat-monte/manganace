...
class CollectionImageService:
    def get_all_of_collection(self, db: Session, collection_id: int) -> List[s.CollectionImage]:
    ...
    def get(self, db: Session, id_: int) -> Optional[s.CollectionImage]:
    ...
    def create(self, db: Session, collection_image_in: s.CollectionImageCreate) -> s.CollectionImage:
        db_collection_image = CRUD.collection_image.create_with_tags(db, collection_image_in)
        return self.construct_collection_image(db_collection_image)

    def update(self, db: Session, id_: int, collection_image_in: s.CollectionImageUpdate) -> Optional[s.CollectionImage]:
        db_collection_image = CRUD.collection_image.get(db, id_)
        if not db_collection_image:
            return None
    ...
    def delete(self, db: Session, id_: int) -> Optional[s.CollectionImage]:
        db_collection_image = CRUD.collection_image.get(db, id_)
        if not db_collection_image:
            return None
    ...
    @staticmethod
    def construct_collection_image(db_collection_image: m.CollectionImage):
        return s.CollectionImage.construct(
