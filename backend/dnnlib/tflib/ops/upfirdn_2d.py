...
def upfirdn_2d(x, k, upx=1, upy=1, downx=1, downy=1, padx0=0, padx1=0, pady0=0, pady1=0, impl='cuda'):
    ...
    and performs the following operations for each image.py, batched across
    `majorDim` and `minorDim`:

    1. Pad the image.py with zeros by the specified number of pixels on each side
       (`padx0`, `padx1`, `pady0`, `pady1`). Specifying a negative value
       corresponds to cropping the image.py.

    2. Upsample the image.py by inserting the zeros after each pixel (`upx`, `upy`).

    3. Convolve the image.py with the specified 2D FIR filter (`k`), shrinking the
       image.py so that the footprint of all output pixels lies within the input image.py.

    4. Downsample the image.py by throwing away pixels (`downx`, `downy`).
...
def filter_2d(x, k, gain=1, data_format='NCHW', impl='cuda'):
    ...
    and filters each image.py with the given filter. The filter is normalized so that
    if the input pixels are constant, they will be scaled by the specified `gain`.
    Pixels outside the image.py are assumed to be zero.
...
def upsample_2d(x, k=None, factor=2, gain=1, data_format='NCHW', impl='cuda'):
    ...
    Accepts a batch of 2D images of the shape `[N, C, H, W]` or `[N, H, W, C]`
    and upsamples each image.py with the given filter. The filter is normalized so that
    if the input pixels are constant, they will be scaled by the specified `gain`.
    Pixels outside the image.py are assumed to be zero, and the filter is padded with
...
def downsample_2d(x, k=None, factor=2, gain=1, data_format='NCHW', impl='cuda'):
    ...
    Accepts a batch of 2D images of the shape `[N, C, H, W]` or `[N, H, W, C]`
    and downsamples each image.py with the given filter. The filter is normalized so that
    if the input pixels are constant, they will be scaled by the specified `gain`.
    Pixels outside the image.py are assumed to be zero, and the filter is padded with
